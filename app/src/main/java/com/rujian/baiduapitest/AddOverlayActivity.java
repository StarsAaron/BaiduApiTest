...
public class AddOverlayActivity extends Activity implements OnClickListener {
	...
	// GroundOptions
	BitmapDescriptor bitmap2 = BitmapDescriptorFactory.fromResource(R.mipmap.csdn_blog);
		...
		bdMap.setOnMarkerClickListener(new OnMarkerClickListener() {
			...
			public boolean onMarkerClick(Marker arg0) {
				if (arg0 == marker1) {
					...
					Toast.makeText(AddOverlayActivity.this, latLng.toString(),Toast.LENGTH_SHORT).show();
				}
				return false;
			}
		});
	...
	private void addMarkerOverlay() {
		...
		// 构建markerOption，用于在地图上添加marker(OverlayOptions是所有覆盖物类的抽象类)
	...
	private void addGroundOverlayOptions() {
		...
		LatLngBounds bounds = new LatLngBounds.Builder()//Builder地理范围构造器
				.include(southwest)
				.include(northeast).build();// 根据西南和东北坐标得到一个地理范围对象

		GroundOverlayOptions groundOverlayOptions = new GroundOverlayOptions()
				.image(bitmap2)// 显示的图片
				.positionFromBounds(bounds)// 显示的位置
		...
		OnGetGeoCoderResultListener listener = new OnGetGeoCoderResultListener() {
			...
			public void onGetReverseGeoCodeResult(ReverseGeoCodeResult result) {
				if (result == null || result.error != SearchResult.ERRORNO.NO_ERROR) {
					// 没有检测到结果
					Toast.makeText(AddOverlayActivity.this, "抱歉，未能找到结果",Toast.LENGTH_LONG).show();
				}
				Toast.makeText(AddOverlayActivity.this,"位置：" + result.getAddress(), Toast.LENGTH_LONG).show();
			}
