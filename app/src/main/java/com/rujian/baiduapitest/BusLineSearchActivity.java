...
public class BusLineSearchActivity extends Activity implements OnClickListener {
	...
	private void searchBusline() {
		...
		if (buslineIndex >= 0 && buslineIndex < buslineIdList.size() && buslineIdList.size() > 0) {
			...
			if (flag) {
				Toast.makeText(BusLineSearchActivity.this, "检索成功~", Toast.LENGTH_SHORT).show();
			} else {
				Toast.makeText(BusLineSearchActivity.this, "检索失败~", Toast.LENGTH_SHORT).show();
			}
			buslineIndex++;
		}
	}
	...
	OnGetPoiSearchResultListener poiSearchResultListener = new OnGetPoiSearchResultListener() {
		...
		public void onGetPoiResult(PoiResult poiResult) {
			if (poiResult == null || poiResult.error == SearchResult.ERRORNO.RESULT_NOT_FOUND) {// 没有找到检索结果
				Toast.makeText(BusLineSearchActivity.this, "未找到结果", Toast.LENGTH_LONG).show();
				return;
			}
	...
	OnGetBusLineSearchResultListener busLineSearchResultListener = new OnGetBusLineSearchResultListener() {
		...
		public void onGetBusLineResult(BusLineResult busLineResult) {
			if (busLineResult.error != SearchResult.ERRORNO.NO_ERROR) {
				...
				// 公交线路名称
				Toast.makeText(BusLineSearchActivity.this,busLineResult.getBusLineName(), Toast.LENGTH_SHORT).show();
			}
		}
	};
